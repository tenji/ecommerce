...
/**
 * Job - Batch answer surveys of Detroit Free Press on specific days
 * 
 * @author TENJI
 * @since
 * @date 2014-8-15
 */
public class DetroitFreePressJob implements Job {
	
	Logger logger = Logger.getLogger(DetroitFreePressJob.class);
	...
	public void execute(JobExecutionContext jec) throws JobExecutionException {
		logger.error("---------- Detroit Free Press ----------");
		...
		// ramId of the Detroit Free Press is 19
		List<RamAccount> ramAccounts = ramAccountService.getListByPropertyName("ram.id", (long) 19);
		...
		for (RamAccount ramAccount : ramAccounts) {
			if (null != logAdminRamAccountService.getLogByDate(new Date(), ramAccount.getId()) ||
				...
				// If this account was operated today or yesterday, break!
				break ;
			}
			try {
