...
<html>
  <head>
    <title>Ram Accounts Lists - ${projectName }</title>
...
function batch(batchButton) {
	...
	} else if ("batchCheck" == batchButton.value) {
		// Batch check unanswered survey numbers of all accounts
		// 发送AJAX请求
		var url = "${path}" + "/admin/affiliate/ramAccount/batchCheck";
	  	$.ajax({
	  		url: url, dataType: 'json', data: {"ids" : ids}, type: 'POST', async: true, 
	  		success: function(list) {
	  			EC.createNoty("success", "Successfully!");
...
// Answer survey of specific account(Extend)
function answerExtendInit(buttonObj) {
	EC.showLoadingLayer(); // 添加加载层
	// 发送AJAX请求
	var url = "${path}" + "/admin/affiliate/ramAccount/answerExtend";
  	$.ajax({
  		url: url, dataType: 'json',	data: {"id": buttonObj.value}, type: 'POST', async: false, 
  		success: function(list) {
  			EC.createNoty("success", "更新成功");
  			$(buttonObj).closest("tr").find(".pointsBeforeOpr").html(list.pointsBeforeOpr); // Change points before operation
  			$(buttonObj).closest("tr").find(".pointsAfterOpr").html(list.pointsAfterOpr); // Change points after operation
  			// $(buttonObj).closest("tr").find(".latestCheckTime").html(list.latestCheckTime); // Change latest
  			// $(buttonObj).closest("tr").find(".latestOprTime").html(list.latestOprTime); // Change latest operation time
...
<body>
	...
	<div class="main-container" id="main-container">
		...
		<div class="main-container-inner">
			...
			<c:set var="subMenuName" value="ramAccount"></c:set>
			...
			<div class="main-content">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Ram Accounts</li>
					</ul><!-- .breadcrumb -->
								...
								<div class="widget-body">
									<div class="widget-main">
										<form class="form-inline" action="${path }/admin/affiliate/ramAccount/list">
											...
											<label class="" for="ram.id">Ram Sites: </label>
											<div class="form-group">
												<select class="form-control" name="ram.id" size="1">
													<option value="" selected="selected">Please select specific ram site</option>
													<c:forEach items="${ramList }" var="ram">
														<option value='<c:out value="${ram.id }"></c:out>' <c:if test="${ram.id == model.ram.id }">selected</c:if> ><c:out value="${ram.name }"></c:out></option>
													</c:forEach>
												</select>
											</div>&nbsp;&nbsp;&nbsp;
											<label class="" for="redeemStatus">Redeem Status: </label>
											<div class="form-group">
												<select class="form-control" name="redeemStatus" size="1">
													<option value="" selected="selected">Please select redeem status</option>
													<option value="0" <c:if test="${model.redeemStatus == 0 }">selected</c:if>>No Redeem</option>
													<option value="1" <c:if test="${model.redeemStatus == 1 }">selected</c:if>>Redeeming</option>
												</select>
											</div>&nbsp;&nbsp;&nbsp;
											<br/><br/>
											<label class="" for="latestOprTime">Latest Opr Time: </label>
											<div class="form-group">
												<input type="text" placeholder="yyyy-MM-dd" name="latestOprStartTime"
													data-date-format="yyyy-mm-dd " id="latestOprStartTime" class="date-picker"
													value="<fmt:formatDate value="${model.latestOprStartTime }" pattern="yy-MM-dd HH:mm:ss" />">
											</div>to
											<div class="form-group">
												<input type="text" placeholder="yyyy-MM-dd" name="latestOprEndTime"
													data-date-format="yyyy-mm-dd" id="latestOprEndTime" class="date-picker"
													value="<fmt:formatDate value="${model.latestOprEndTime }" pattern="yy-MM-dd HH:mm:ss" />">
											</div>
											<br/><br/>
							...
							<div class="dataTables_wrapper">
								<div class="row">
									<div class="col-sm-6">
										...
										<button type="button" class="btn btn-primary btn-xs batch" value="batchCheck" disabled onclick="batch(this)">Batch Check</button>
									...
									<div class="col-sm-6">
										<div class="dataTables_length pull-right">
											...
											<select name="sample-table-2_length" size="1" id="pageSize">
												...
												<option value="200" <c:if test="${param.pageSize == 200 }">selected</c:if>>200</option>
								...
								<thead>
									<tr>
										...
										<th style="">No</th>
										<th style="">EMail</th>
										<th style="">Ram Site</th>
										<th style="">UnAnswered</th>
										<th style="">Points Before Opr</th>
										<th style="">Points After Opr</th>
										<th style="">Redeem Status</th>
										<th style="">Latest Redeem Time</th>
										<th style="">Latest Check Time</th>
								...
								<tbody>
									<c:forEach items="${pageBean.list }" var="model" varStatus="st">
										<tr class="active">
											...
											<td class="unanswered">${model.unansweredNums }</td>
											<td class="pointsBeforeOpr">${model.pointsBeforeOpr }</td>
											<td class="pointsAfterOpr">${model.pointsAfterOpr }</td>
											<td class="redeemStatus"><c:choose><c:when test="${model.redeemStatus == 0 }">No Redeem</c:when><c:otherwise>Redeeming</c:otherwise></c:choose></td>
											<td><fmt:formatDate value="${model.latestRedeemTime }" pattern="yy-MM-dd HH:mm:ss" /></td>
											<td class="latestCheckTime"><fmt:formatDate value="${model.latestCheckTime }" pattern="yy-MM-dd HH:mm:ss" /></td>
											<td class="latestOprTime"><fmt:formatDate value="${model.latestOprTime }" pattern="yy-MM-dd HH:mm:ss" /></td>
											<td>
											...
											<button type="button" class="btn btn-primary btn-xs"
												value='${model.id }'
												onclick="answerExtendInit(this)">AnswerEx</button>
											<button type="button" class="btn btn-primary btn-xs"
												value='${model.id }'
